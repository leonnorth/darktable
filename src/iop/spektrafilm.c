...
typedef enum dt_iop_spektrafilm_output_compress_t
{
  DT_SPEKTRAFILM_OC_OFF = 0,   // $DESCRIPTION: "off"
  DT_SPEKTRAFILM_OC_OKLCH = 1, // $DESCRIPTION: "OKLCh"
  DT_SPEKTRAFILM_OC_ACES = 2,  // $DESCRIPTION: "ACES RGC"
} dt_iop_spektrafilm_output_compress_t;
...
typedef struct dt_iop_spektrafilm_params_t
{
  ...
  /* --- advanced options (v2): engine params previously fixed at defaults --- */
  /* film development */
  float density_curve_gamma;   // $MIN: 0.5 $MAX: 2.0 $DEFAULT: 1.0 $DESCRIPTION: "film gamma"
  float inhibition_samelayer;  // $MIN: 0.0 $MAX: 2.0 $DEFAULT: 1.0 $DESCRIPTION: "DIR inhibition (same layer)"
  float inhibition_interlayer; // $MIN: 0.0 $MAX: 2.0 $DEFAULT: 1.0 $DESCRIPTION: "DIR inhibition (interlayer)"
  /* enlarger / print */
  gboolean normalize_print_exposure; // $DEFAULT: TRUE $DESCRIPTION: "normalize print exposure"
  float preflash_exposure;   // $MIN: 0.0 $MAX: 2.0 $DEFAULT: 0.0 $DESCRIPTION: "preflash exposure"
  float preflash_filter_y;   // $MIN: -60.0 $MAX: 60.0 $DEFAULT: 0.0 $DESCRIPTION: "preflash Y"
  float preflash_filter_m;   // $MIN: -60.0 $MAX: 60.0 $DEFAULT: 0.0 $DESCRIPTION: "preflash M"
  gboolean neutral_override; // $DEFAULT: FALSE $DESCRIPTION: "override print neutral"
  float filter_c_neutral;    // $MIN: 0.0 $MAX: 200.0 $DEFAULT: 0.0 $DESCRIPTION: "neutral C"
  float filter_m_neutral;    // $MIN: 0.0 $MAX: 200.0 $DEFAULT: 65.0 $DESCRIPTION: "neutral M"
  float filter_y_neutral;    // $MIN: 0.0 $MAX: 200.0 $DEFAULT: 55.0 $DESCRIPTION: "neutral Y"
  /* print curve morph */
  float print_gamma_fast; // $MIN: 0.5 $MAX: 2.0 $DEFAULT: 1.0 $DESCRIPTION: "print gamma (highlights)"
  float print_gamma_slow; // $MIN: 0.5 $MAX: 2.0 $DEFAULT: 1.0 $DESCRIPTION: "print gamma (shadows)"
  ...
  /* color management */
  gboolean input_gamut_compress; // $DEFAULT: TRUE $DESCRIPTION: "input gamut compression"
  dt_iop_spektrafilm_output_compress_t output_compress; // $DEFAULT: DT_SPEKTRAFILM_OC_OKLCH $DESCRIPTION: "output compression"
...
typedef struct dt_iop_spektrafilm_gui_data_t
{
  ...
  GtkWidget *density_curve_gamma, *inhibition_samelayer, *inhibition_interlayer;
  GtkWidget *normalize_print_exposure, *preflash_exposure, *preflash_filter_y, *preflash_filter_m;
  GtkWidget *neutral_override, *filter_c_neutral, *filter_m_neutral, *filter_y_neutral;
  GtkWidget *print_gamma_fast, *print_gamma_slow, *print_gamma_r, *print_gamma_g, *print_gamma_b;
  GtkWidget *input_gamut_compress, *output_compress;
                  ...
                  int *new_version)
{
  ...
  if(old_version == 1)
  {
    const dt_iop_spektrafilm_params_v1_t *o = old_params;
    dt_iop_spektrafilm_params_t *n = malloc(sizeof(dt_iop_spektrafilm_params_t));

    /* copy the v1 fields verbatim */
    ...
    /* new v2 fields -> defaults (identity, so old edits look unchanged) */
    n->density_curve_gamma = 1.0f;
    n->inhibition_samelayer = 1.0f;
    n->inhibition_interlayer = 1.0f;
    n->normalize_print_exposure = TRUE;
    n->preflash_exposure = 0.0f;
    n->preflash_filter_y = 0.0f;
    n->preflash_filter_m = 0.0f;
    n->neutral_override = FALSE;
    n->filter_c_neutral = 0.0f;
    n->filter_m_neutral = 65.0f;
    n->filter_y_neutral = 55.0f;
    n->print_gamma_fast = 1.0f;
    n->print_gamma_slow = 1.0f;
    n->print_gamma_r = 1.0f;
    n->print_gamma_g = 1.0f;
    n->print_gamma_b = 1.0f;
    n->input_gamut_compress = TRUE;
    n->output_compress = DT_SPEKTRAFILM_OC_OKLCH;
                             ...
                             const dt_iop_order_iccprofile_info_t *work_profile)
{
  ...
  key = _mix64(key, &p->density_curve_gamma, sizeof p->density_curve_gamma);
  key = _mix64(key, &p->inhibition_samelayer, sizeof p->inhibition_samelayer);
  key = _mix64(key, &p->inhibition_interlayer, sizeof p->inhibition_interlayer);
  key = _mix64(key, &p->normalize_print_exposure, sizeof p->normalize_print_exposure);
  key = _mix64(key, &p->preflash_exposure, sizeof p->preflash_exposure);
  key = _mix64(key, &p->preflash_filter_y, sizeof p->preflash_filter_y);
  key = _mix64(key, &p->preflash_filter_m, sizeof p->preflash_filter_m);
  key = _mix64(key, &p->neutral_override, sizeof p->neutral_override);
  key = _mix64(key, &p->filter_c_neutral, sizeof p->filter_c_neutral);
  key = _mix64(key, &p->filter_m_neutral, sizeof p->filter_m_neutral);
  key = _mix64(key, &p->filter_y_neutral, sizeof p->filter_y_neutral);
  key = _mix64(key, &p->print_gamma_fast, sizeof p->print_gamma_fast);
  key = _mix64(key, &p->print_gamma_slow, sizeof p->print_gamma_slow);
  ...
  key = _mix64(key, &p->input_gamut_compress, sizeof p->input_gamut_compress);
  key = _mix64(key, &p->output_compress, sizeof p->output_compress);
  ...
  if(film && (paper || p->scan_film))
  {
    ...
    sp.density_curve_gamma = p->density_curve_gamma;
    sp.inhibition_samelayer = p->inhibition_samelayer;
    sp.inhibition_interlayer = p->inhibition_interlayer;
    /* advanced: enlarger / print */
    sp.normalize_print_exposure = p->normalize_print_exposure;
    sp.preflash_exposure = p->preflash_exposure;
    sp.preflash_y_shift = p->preflash_filter_y;
    sp.preflash_m_shift = p->preflash_filter_m;
    sp.neutral_from_db = !p->neutral_override;
    if(p->neutral_override)
    {
      sp.c_filter_neutral = p->filter_c_neutral;
      sp.m_filter_neutral = p->filter_m_neutral;
      sp.y_filter_neutral = p->filter_y_neutral;
    }
    /* advanced: color management */
    sp.input_gamut_compress = p->input_gamut_compress;
    sp.output_compress = (sf_output_compress_t)p->output_compress;
    /* print curve morph: existing print_contrast plus per-branch/per-channel */
    sp.morph_gamma = p->print_contrast;
    sp.morph_gamma_fast = p->print_gamma_fast;
    sp.morph_gamma_slow = p->print_gamma_slow;
    sp.morph_gamma_r = p->print_gamma_r;
    sp.morph_gamma_g = p->print_gamma_g;
    sp.morph_gamma_b = p->print_gamma_b;
    sp.morph_active = (p->print_contrast != 1.0f)
                   || (p->print_gamma_fast != 1.0f) || (p->print_gamma_slow != 1.0f)
                   || (p->print_gamma_r != 1.0f) || (p->print_gamma_g != 1.0f)
                   || (p->print_gamma_b != 1.0f);
...
void gui_update(dt_iop_module_t *self)
{
  ...
  gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(g->normalize_print_exposure), p->normalize_print_exposure);
  gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(g->neutral_override), p->neutral_override);
  gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(g->input_gamut_compress), p->input_gamut_compress);
  _update_print_sensitivity(self);
}
...
void gui_init(dt_iop_module_t *self)
{
  ...
  dt_gui_new_collapsible_section(&g->advanced_section,
                                 "plugins/darkroom/spektrafilm/expand_advanced",
                                 _("advanced options"),
                                 GTK_BOX(self->widget), DT_ACTION(self));
  dt_iop_module_t *sec = DT_IOP_SECTION_FOR_PARAMS(self, N_("advanced"),
                                                   g->advanced_section.container);

  gtk_box_pack_start(GTK_BOX(g->advanced_section.container),
                     dt_ui_section_label_new(_("film development")), FALSE, FALSE, 0);
  g->density_curve_gamma = dt_bauhaus_slider_from_params(sec, "density_curve_gamma");
  gtk_widget_set_tooltip_text(g->density_curve_gamma,
                              _("film density-curve gamma; overall negative contrast (1.0 = film-accurate)"));
  g->inhibition_samelayer = dt_bauhaus_slider_from_params(sec, "inhibition_samelayer");
  gtk_widget_set_tooltip_text(g->inhibition_samelayer,
                              _("DIR coupler inhibition within a layer (affects local contrast/sharpness)"));
  g->inhibition_interlayer = dt_bauhaus_slider_from_params(sec, "inhibition_interlayer");
  gtk_widget_set_tooltip_text(g->inhibition_interlayer,
                              _("DIR coupler inhibition across layers (drives cross-channel saturation)"));

  gtk_box_pack_start(GTK_BOX(g->advanced_section.container),
                     dt_ui_section_label_new(_("enlarger / print")), FALSE, FALSE, 0);
  g->normalize_print_exposure = dt_bauhaus_toggle_from_params(sec, "normalize_print_exposure");
  gtk_widget_set_tooltip_text(g->normalize_print_exposure,
                              _("normalize the print exposure to a fixed reference density"));
  g->preflash_exposure = dt_bauhaus_slider_from_params(sec, "preflash_exposure");
  gtk_widget_set_tooltip_text(g->preflash_exposure,
                              _("pre-flash the paper: lifts shadows and lowers print contrast"));
  g->preflash_filter_y = dt_bauhaus_slider_from_params(sec, "preflash_filter_y");
  dt_bauhaus_slider_set_format(g->preflash_filter_y, _(" CC"));
  gtk_widget_set_tooltip_text(g->preflash_filter_y, _("yellow filtration of the pre-flash"));
  g->preflash_filter_m = dt_bauhaus_slider_from_params(sec, "preflash_filter_m");
  dt_bauhaus_slider_set_format(g->preflash_filter_m, _(" CC"));
  gtk_widget_set_tooltip_text(g->preflash_filter_m, _("magenta filtration of the pre-flash"));
  g->neutral_override = dt_bauhaus_toggle_from_params(sec, "neutral_override");
  gtk_widget_set_tooltip_text(g->neutral_override,
                              _("set the print neutral filtration manually instead of from the profile database"));
  g->filter_c_neutral = dt_bauhaus_slider_from_params(sec, "filter_c_neutral");
  dt_bauhaus_slider_set_format(g->filter_c_neutral, _(" CC"));
  gtk_widget_set_tooltip_text(g->filter_c_neutral, _("cyan neutral filtration (only if override is on)"));
  g->filter_m_neutral = dt_bauhaus_slider_from_params(sec, "filter_m_neutral");
  dt_bauhaus_slider_set_format(g->filter_m_neutral, _(" CC"));
  gtk_widget_set_tooltip_text(g->filter_m_neutral, _("magenta neutral filtration (only if override is on)"));
  g->filter_y_neutral = dt_bauhaus_slider_from_params(sec, "filter_y_neutral");
  dt_bauhaus_slider_set_format(g->filter_y_neutral, _(" CC"));
  gtk_widget_set_tooltip_text(g->filter_y_neutral, _("yellow neutral filtration (only if override is on)"));

  gtk_box_pack_start(GTK_BOX(g->advanced_section.container),
                     dt_ui_section_label_new(_("print curve")), FALSE, FALSE, 0);
  g->print_gamma_fast = dt_bauhaus_slider_from_params(sec, "print_gamma_fast");
  gtk_widget_set_tooltip_text(g->print_gamma_fast, _("print contrast in the highlights (fast curve branch)"));
  g->print_gamma_slow = dt_bauhaus_slider_from_params(sec, "print_gamma_slow");
  gtk_widget_set_tooltip_text(g->print_gamma_slow, _("print contrast in the shadows (slow curve branch)"));
  g->print_gamma_r = dt_bauhaus_slider_from_params(sec, "print_gamma_r");
  gtk_widget_set_tooltip_text(g->print_gamma_r, _("per-channel print contrast: red"));
  g->print_gamma_g = dt_bauhaus_slider_from_params(sec, "print_gamma_g");
  gtk_widget_set_tooltip_text(g->print_gamma_g, _("per-channel print contrast: green"));
  g->print_gamma_b = dt_bauhaus_slider_from_params(sec, "print_gamma_b");
  gtk_widget_set_tooltip_text(g->print_gamma_b, _("per-channel print contrast: blue"));

  gtk_box_pack_start(GTK_BOX(g->advanced_section.container),
                     dt_ui_section_label_new(_("color management")), FALSE, FALSE, 0);
  g->input_gamut_compress = dt_bauhaus_toggle_from_params(sec, "input_gamut_compress");
  gtk_widget_set_tooltip_text(g->input_gamut_compress,
                              _("pull out-of-locus input colors inside the visible spectrum before simulation"));
  g->output_compress = dt_bauhaus_combobox_from_params(sec, "output_compress");
  gtk_widget_set_tooltip_text(g->output_compress,
                              _("compress out-of-gamut colors and above-white tones into the output space"));
}
